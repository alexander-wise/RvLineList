...
:allowBlends => [0,1,2,3,4,5,6,7,8], #number of blends allowed with each line, e.g. 0 for single lines only, 1 for doublets only, [0,1] for singlets and doublets
:overlap_cutoff => 3.0e-5, #distance between lines required for them to be categorized as a blend, expressed as a fraction of the speed of light
:depth_cutoff => 0.01,
...
:quant => "100", #quantile for stability of fit params for empirical masks, expressed as a string out of 100
